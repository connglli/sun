...
TEST(IGVParserTest, ParseSimpleReturnEdges) {
  ...
  EXPECT_EQ(ret->input(0)->opcode(), Opcode::kStart);

  // Input 1: value from ConI (node 10)
  EXPECT_EQ(ret->input(1)->id(), 10);
  EXPECT_EQ(ret->input(1)->opcode(), Opcode::kConI);
}
...
TEST(IGVParserTest, ParseSimpleAdd) {
  ...
  EXPECT_EQ(parm0->opcode(), Opcode::kParm);
  EXPECT_TRUE(parm0->has_prop("index"));
  EXPECT_EQ(std::get<int32_t>(parm0->prop("index")), 0);
  EXPECT_TRUE(parm0->has_prop("type"));
  EXPECT_EQ(std::get<std::string>(parm0->prop("type")), "int");

  // Verify second Parm node
  Node* parm1 = graph->node(11);
  ASSERT_NE(parm1, nullptr);
  EXPECT_EQ(parm1->opcode(), Opcode::kParm);
  EXPECT_TRUE(parm1->has_prop("index"));
  EXPECT_EQ(std::get<int32_t>(parm1->prop("index")), 1);
...
TEST(IGVParserTest, ParseSimpleAddEdges) {
  ...
  EXPECT_EQ(addI->input(0)->opcode(), Opcode::kParm);

  // Input 1: Parm node 11
  EXPECT_EQ(addI->input(1)->id(), 11);
  EXPECT_EQ(addI->input(1)->opcode(), Opcode::kParm);
  ...
  EXPECT_EQ(ret->input(1)->opcode(), Opcode::kAddI);
}
